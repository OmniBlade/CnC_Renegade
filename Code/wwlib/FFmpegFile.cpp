...
{
	WWASSERT_PRINT(m_file == nullptr, ("already open"));
...
// This is required for FFmpeg older than 4.0 -> deprecated afterwards though
	...
	av_register_all();
#endif
	m_factory = _TheFileFactory;
	m_file = _TheFileFactory->Get_File(file);
	...
	m_avioCtx = avio_alloc_context(buffer, avio_ctx_buffer_size, 0, m_file, &readPacket, nullptr, nullptr);
...
{
	WWASSERT_PRINT(m_fmtCtx != nullptr, ("null format context"));
	WWASSERT_PRINT(m_packet != nullptr, ("null packet pointer"));
	...
	WWASSERT_PRINT(m_streams.size() > stream_idx, ("stream index out of bounds"));
...
bool FFmpegFile::hasVideo() const
{
...
unsigned FFmpegFile::getFrameTime() const
{
	const FFmpegStream *stream = findMatch(AVMEDIA_TYPE_VIDEO);
	if (stream == nullptr)
		return 0u;
